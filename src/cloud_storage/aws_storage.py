...
class SimpleStorageService:
    ...
    def get_bucket(self, bucket_name: str) -> Bucket:
        ...
        """
        logging.debug("Entered the get_bucket method of SimpleStorageService class")
        try:
            bucket = self.s3_client.resource.Bucket(bucket_name)
            logging.debug("Exited the get_bucket method of SimpleStorageService class")
            return bucket
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def create_folder(self, folder_name: str, bucket_name: str) -> None:
        ...
        try:
            ...
            self.s3_client.resource.Object(bucket_name, folder_name).load()
        except ClientError as e:
            # If folder does not exist, create it
            if e.response["Error"]["Code"] == "404":
                folder_obj = folder_name + "/"
                self.s3_client.client.put_object(Bucket=bucket_name, Key=folder_obj)
    ...
    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = True):
        ...
        try:
            ...
            self.s3_client.resource.meta.client.upload_file(from_filename, bucket_name, to_filename)
...
class ModelS3(SimpleStorageService):
    """
    ...
    def load_model(self) -> MyModel:
        ...
        """
        try:
            file_object = self.get_file_object(self.model_path, self.bucket_name)
    ...
    def save_model(self, from_file, remove:bool=False)->None:
        ...
        """
        try:
            self.upload_file(from_file, to_filename=self.model_path, bucket_name=self.bucket_name, remove=remove)
        except Exception as e:
            raise MyException(e, sys)
